...
class Result
{
    ...
    {
        var n = container[0].Count;
        
        var balls_per_cont = new long[n];
        var balls_per_type = new long[n];
        
        for (var i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
            {
                balls_per_cont[i] += container[i][j];
                balls_per_type[j] += container[i][j];
            }
        
        Array.Sort(balls_per_cont);
        Array.Sort(balls_per_type);
        
        return balls_per_type.SequenceEqual(balls_per_cont) ? "Possible" : "Impossible";
    }
}
